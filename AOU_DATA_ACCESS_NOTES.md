...
4. **Attempt the region-slice** on that same CRAM (now via the gcsfuse-mounted local path) with explicit `-T` pointing at the public Broad hg38 FASTA (section 3, still reachable via plain `gs://` since it's a public non-requester-pays bucket — confirmed separately that plain `gs://` access works fine for public buckets, it's specifically requester-pays buckets that need the gcsfuse workaround).
