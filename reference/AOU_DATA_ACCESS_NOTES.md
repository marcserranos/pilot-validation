...
**[CONFIRMED, HIGH, 2026-07-18] Found — "All of Us Genomics & Multi-omics Quality Report," v9 (`C2025Q4R6`).** Renamed from the earlier "Genomic Quality Report" title to reflect added multi-omics content. Public, **no Workbench login required**: [landing page](https://support.researchallofus.org/hc/en-us/articles/50655639562900-All-of-Us-Genomics-Multi-omics-Quality-Report), [PDF direct link](https://support.researchallofus.org/hc/en-us/article_attachments/50656363463700) (~13.4MB, 145pp). `support.researchallofus.org` 403s a plain curl/generic user-agent — needed a real browser session to reach it, but that's a bot-check, not an auth wall.

Ground truth: 8 GIAB/NIST reference samples (HG-001/NA12878 ×2 replicates, HG-002/3/4 Ashkenazi trio, HG-005 Han/Chinese ancestry son). **Table 8 (p.27-28), srWGS SNP/indel joint callset vs. GIAB v4.2.1 high-confidence regions:** SNV sensitivity 0.984–0.9865 / precision 0.9991–0.9997; indel sensitivity 0.9708–0.9904 / precision 0.9961–0.9988, across all 8 samples.

**HLA is NOT locus-specifically benchmarked in this report.** Mentioned only twice: (a) named as an existing auxiliary data product in the overview list ("...HLA variant calls, and challenging medically relevant gene calls..."); (b) RNA-seq alignment notes HLA is *excluded* (along with ALT/decoy contigs) from the hg38 build used for RNA alignment. The "challenging medically relevant genes" QC section (p.71-72) covers KCNE1/CBS/MAP2K3 only — no chr6 MHC/classical HLA gene-specific sensitivity data anywhere in the document. **Practical implication: this project's own n=60 3-way pilot is, as far as we've found, the only HLA-specific reliability evidence that exists for AoU's HLA callset** — not a gap in our own work, a real gap in the field's documentation of this dataset.
...
**[HIGH, important operational finding] Direct remote (`gs://`) access from *viewer tools* is explicitly blocked by design, and AoU recommends localizing files first.** The AoU support article "Using the Integrative Genomics Viewer (IGV)" states that although IGV nominally supports direct GCS access, **"this feature unfortunately cannot currently be used in the Workbench due to data exfiltration controls,"** and the recommended workaround is to `gsutil cp` the CRAM + `.crai` into the current VM/environment first, then work locally. I could not fetch the full article body (403'd), so I don't have the exact accompanying command text — this is from a search-result snippet summarizing it, so treat the precise wording as **[MED]** even though the underlying fact (exfiltration controls exist and affect genomic file access patterns) is corroborated separately by AoU's "Researcher Workbench" security documentation, which describes a **perimeter/VPC-Service-Controls-style policy** restricting "copy, transfer and retrieval of data" across the workspace's security perimeter boundary **[HIGH — this general perimeter-control fact is stated directly in AoU workbench security docs]**.
  ...
  - **RESOLVED (2026-07-08), HIGH — real live test result.** The "headline risk" is answered: reading controlled-tier CRAM/BAM data directly from inside the VM is **not** blocked by exfiltration controls — but htslib's own built-in `gs://` support (samtools 1.21, this pixi env) does **not** reliably work against this specific bucket because it's **requester-pays**, and htslib doesn't cleanly support attaching a billing project to a `gs://` request:
    - Plain `samtools view -H gs://vwb-aou-datasets-controlled/...cram` with just `GCS_OAUTH_TOKEN` set → fails immediately with `[E::hts_open_format] ... Invalid argument` (request never even gets billing info attached).
    - Adding `export GCS_REQUESTER_PAYS_PROJECT=<billing-project>` *changes* the error to `Operation not permitted` (exit code 1) — so the variable does something, but doesn't get you through. Confirmed by a clean redirect-to-file test (`> out.txt 2>err.txt; echo $?`), not by eyeballing terminal scrollback — **a `tail`/`grep`-filtered pipe silently swallowed the real error text on an earlier attempt and looked like stale/duplicate success output; always redirect to a file and check the exit code when debugging remote access, don't trust a filtered pipe.**
    - **The fix that works: mount the bucket with `gcsfuse`, not raw `gs://` URLs.** `gcsfuse` was already preinstalled on the VM (v3.10.0, no pixi/apt install needed):
      ```bash
      ...
      ```
      **`--implicit-dirs` is required** — without it, nested paths like `pooled/wgs/cram/v8_base/<file>.cram` silently 404 (`ls` on the mount root returns empty) because GCS has no real directories; AoU's data was written without explicit directory-placeholder objects, so gcsfuse won't infer the `pooled/wgs/cram/...` structure unless told to. This version of `gcsfuse` daemonizes automatically after printing "File system has been successfully mounted." (returns you to the prompt) — no need for a second terminal/foreground process despite older gcsfuse docs suggesting otherwise.
      - Once mounted, every file under the bucket is just a normal local path (e.g. `~/mnt/aou-controlled/pooled/wgs/cram/v8_base/wgs_<person_id>.cram`) — plain `samtools view -H <path>` works with zero further auth/env-var complexity, confirmed against a real person's CRAM (real `@HD`/`@RG`/`@PG` lines, DRAGEN-aligned, BCM, NovaSeq 6000).
      - **This is now the standard recommended access method for all CRAM/BAM reads in this project** — use the gcsfuse mount, not raw `gs://` URLs, for both srWGS CRAMs and lrWGS BAMs.
...
**Path — gsutil-verified 2026-07-10:** `gs://vwb-aou-datasets-controlled/v9/wgs/short_read/snpindel/aux/ancestry/ancestry_preds.tsv`, confirmed present alongside `eigenvalues.txt`, `loadings.ht.tar.gz`, `merged_sites_only_intersection.vcf.bgz(.tbi)`, `preds_oth.html`, `rf_classifier.pkl`, `training_pca.tsv` — matches the source PDF's "Auxiliary Data" list (p.4/14-16) exactly, sibling directory to `hla_variants/`. Full Table 9 schema (`research_id`, `ancestry_pred`, `probabilities` array ordered AFR/AMR/EAS/EUR/MID/SAS, `pca_features`) is in the source PDF pp.14-16.
...
2. ~~Browse the confirmed genomics bucket directly~~ — **done and resolved (2026-07-08)**. The bucket is **requester-pays** — every `gsutil`/`gcloud storage` call against it needs `-u wb-glacial-potato-8710` (your own workspace project bills) or it 400s with `BadRequestException: Bucket is a requester pays bucket but no user project provided`. Confirmed paths:
   - srWGS CRAM manifest: `gs://vwb-aou-datasets-controlled/v9/wgs/cram/manifest.csv` (`person_id,cram_uri,cram_index_uri`)
   - lrWGS manifest: `gs://vwb-aou-datasets-controlled/v9/wgs/long_read/manifest.tsv` (`research_id,center,platform,grch38_bam,grch38_bai,...`)
   - AoU native srWGS HLA calls: `gs://vwb-aou-datasets-controlled/v9/wgs/short_read/snpindel/hla_variants/hla_genotypes.tsv`
   - Internal structure: `<bucket>/v9/wgs/{cram,short_read,long_read}/` is the current-CDR index; `<bucket>/pooled/wgs/cram/{v7_base,v8_base,v9_delta}/` and `<bucket>/pooled/longreads/v9_delta/` hold the physical files (incremental storage across releases) — always resolve via the `v9/` manifests, don't hand-construct `pooled/` paths.
   - `hla_genotypes.tsv` schema confirmed (331 MiB, likely near-full srWGS cohort coverage): `research_id` + 30 gene-pair columns (`A_1/A_2, B_1/B_2, C_1/C_2, DRB1_1/2, DQA1_1/2, DQB1_1/2, DPA1_1/2, DPB1_1/2`, plus 22 more non-classical/pseudogenes: `DMA, DMB, DOA, DOB, DRA, DRB2-9, DPA2, E, F, G, H, J, K, L, V, Y`). **Scope note: restrict comparisons to the 8 classical genes SpecHLA/SpecImmune actually call** — AoU's ensemble types a much broader panel than our pipeline does. Allele resolution is variable (2-field to 3-field depending on evidence), not a fixed field count.
3. ~~On one participant's srWGS CRAM, confirm read access~~ — **done and resolved (2026-07-08)**. Direct `gs://` access via htslib doesn't work against this requester-pays bucket (two different errors across attempts); the working method is a `gcsfuse --billing-project ... --implicit-dirs` mount, then treat the path as a normal local file. See section 1 above for the full recipe. Confirmed live against a real CRAM (person 1017156): `@HD`/`@RG`/`@PG` all present and correct, DRAGEN/BCM/NovaSeq 6000, contig naming is `chr`-prefixed as expected.
4. ~~Attempt the region-slice~~ — **done (2026-07-08)**. `-T` against the `gs://` public reference also failed (different error than the CRAM's — see CURRENT_SPRINT.md quirk #11), fixed the same way: downloaded the ~3GB reference once to local disk (`~/ref/`) instead of fighting remote access further. Region slicing itself works fine once the reference is local.
5. ~~Repeat for lrWGS BAM~~ — **done**. Simpler than srWGS as expected, no `-T` needed.
6. ~~Run `samtools fastq` on both slices~~ — **done**, wrapped into a reusable script, `slice_and_fastq.sh` in the repo root. Skipped SpecHLA/SpecImmune's own `ExtractHLAread.sh`/`ExtractReads.sh` scripts entirely — the manual region-slice-then-fastq approach worked cleanly and both typing tools accepted the resulting FASTQs without issue.
7. ~~Scale to the 2-3 person smoke test~~ — **in progress**. Person 1017156's full 3-way bake-off is complete (see CURRENT_SPRINT.md). Two more people (2522883, 1253627) have SpecImmune-LR done, SpecHLA-SR queued as the literal next step. See CURRENT_SPRINT.md's "Pick up here" block for exact commands.
